...
func (s *Store) MoneyKeyGaps(ctx context.Context, spaceID uuid.UUID) ([]MoneyKeyGap, error) {
	rows, err := s.pool.Query(ctx, "SELECT m.user_id, d.id, d.public_key, u.name, u.username FROM space_members m JOIN spaces sp ON sp.id = m.space_id JOIN users u ON u.id = m.user_id JOIN devices d ON d.user_id = m.user_id AND d.status = 'active' LEFT JOIN space_keys k ON k.space_id = m.space_id AND k.key_epoch = sp.key_epoch AND k.user_id = m.user_id AND k.device_id = d.id WHERE m.space_id = $1 AND k.id IS NULL UNION ALL SELECT m.user_id, NULL, u.escrow_public_key, u.name, u.username FROM space_members m JOIN spaces sp ON sp.id = m.space_id JOIN users u ON u.id = m.user_id LEFT JOIN space_keys k ON k.space_id = m.space_id AND k.key_epoch = sp.key_epoch AND k.user_id = m.user_id AND k.device_id IS NULL WHERE m.space_id = $1 AND u.escrow_public_key IS NOT NULL AND k.id IS NULL ORDER BY 1, 2 NULLS LAST", spaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
