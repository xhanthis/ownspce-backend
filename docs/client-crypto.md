...
| Recovery identity | BIP39 24-word phrase → seed → X25519 keypair | Deterministic. Only the public half is uploaded (`PATCH /me` → `recoveryPublicKey`). |
...
**Recovery phrase** — the new device signs in (`pending`), the user enters the 24 words, the client derives the recovery keypair, calls `GET /spaces?includeRecoveryKeys=true`, unwraps each `recoveryWrappedKey` locally, re-wraps each space key to its own device key, and calls `POST /devices/{id}/approve` on itself with `recovery: true`.
